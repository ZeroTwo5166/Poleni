  ...
  return (
    <section id="calculator" className="relative py-32 px-6 overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        {/*
          Key fix: the grid itself is plain — no motion on the row wrapper.
          Each column is a motion.div for the enter/exit animation,
          but sticky is on a plain inner div INSIDE the motion wrapper,
          not on the motion.div itself. This separates the compositor
          layer (transform) from the scroll-position layer (sticky),
          so the browser can handle both on the GPU without repainting.
        */}
                  ...
                  {WEBSITE_TYPES.map((type) => (
                    ...
                    >
                      ...
                      <span className="relative z-10 flex flex-col gap-1">
                        ...
                        <span
                          className={`text-sm font-medium ${websiteType === type.id ? "text-indigo-400" : ""}`}
                          style={websiteType !== type.id ? { color: "var(--text-secondary)" } : {}}
                        >
                          {type.label}
                        </span>
